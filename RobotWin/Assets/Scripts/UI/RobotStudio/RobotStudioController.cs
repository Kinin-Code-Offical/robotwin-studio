...
{
    public class RobotStudioController : MonoBehaviour
    {
        ...
        private RobotStudioPackage _robotPackage = new RobotStudioPackage();
        private string _robotPackagePath = string.Empty;
        ...
        private bool _assemblyDragging;
        private bool _assemblyRotating;
        private Vector3 _assemblyDragOffset;
        private Vector2 _assemblyRotateAnchor;
        private float _assemblyRotateStartYaw;
        private ComponentCatalog.Item _pendingCatalogItem;
        private bool _pendingCatalogDrag;
        private GameObject _assemblyRoot;
        ...
        private Material _pinIndicatorMaterial;

        private Button _leftTabAssemblyBtn;
        private Button _leftTabEnvironmentBtn;
        private Button _leftTabWiringBtn;
        private VisualElement _leftPanelAssembly;
        private VisualElement _leftPanelEnvironment;
        private VisualElement _leftPanelWiring;
        private Button _assemblySelectBtn;
        private Button _assemblyMoveBtn;
        private Button _assemblyRotateBtn;
        private Button _assemblyNailBtn;
        private Button _assemblyBreadboardBtn;
        ...
        private Slider _environmentFrictionSlider;
        private TextField _wireYoungField;
        private TextField _wireMaxStressField;
        private TextField _wireDampingField;
        private Button _wireApplyBtn;
        ...
        private void OnDestroy()
        {
            ...
            DestroyCachedMaterial(ref _assemblyFloorMaterial);
            DestroyCachedMaterial(ref _placementGhostMaterial);
            DestroyCachedMaterial(ref _pinIndicatorMaterial);
        }

        private void ClearTextureCache()
        {
            if (_textureCache.Count == 0) return;
        ...
        private void BindUi()
        {
            ...
            _leftTabAssemblyBtn = _root.Q<Button>("LeftTabAssemblyBtn");
            _leftTabEnvironmentBtn = _root.Q<Button>("LeftTabEnvironmentBtn");
            _leftTabWiringBtn = _root.Q<Button>("LeftTabWiringBtn");
            ...
            _assemblySelectBtn = _root.Q<Button>("AssemblySelectBtn");
            _assemblyMoveBtn = _root.Q<Button>("AssemblyMoveBtn");
            _assemblyRotateBtn = _root.Q<Button>("AssemblyRotateBtn");
            _assemblyNailBtn = _root.Q<Button>("AssemblyNailBtn");
            _assemblyBreadboardBtn = _root.Q<Button>("AssemblyBreadboardBtn");
            ...
            _environmentFrictionSlider = _root.Q<Slider>("EnvironmentFrictionSlider");
            _wireYoungField = _root.Q<TextField>("WireYoungField");
            _wireMaxStressField = _root.Q<TextField>("WireMaxStressField");
            _wireDampingField = _root.Q<TextField>("WireDampingField");
            _wireApplyBtn = _root.Q<Button>("WireApplyBtn");
            ...
            if (_assemblySelectBtn != null) _assemblySelectBtn.clicked += () => SetAssemblyTool(AssemblyTool.Select);
            if (_assemblyMoveBtn != null) _assemblyMoveBtn.clicked += () => SetAssemblyTool(AssemblyTool.Move);
            if (_assemblyRotateBtn != null) _assemblyRotateBtn.clicked += () => SetAssemblyTool(AssemblyTool.Rotate);
            if (_assemblyNailBtn != null) _assemblyNailBtn.clicked += () => SetAssemblyTool(AssemblyTool.Nail);
            if (_assemblyBreadboardBtn != null) _assemblyBreadboardBtn.clicked += AutoArrangeBreadboard;
            if (_assemblyFrameBtn != null) _assemblyFrameBtn.clicked += FrameAssembly;
            if (_assemblySnapToggle != null) _assemblySnapToggle.RegisterValueChangedCallback(_ => UpdateAssemblySnap());
            if (_assemblySnapField != null && string.IsNullOrWhiteSpace(_assemblySnapField.value))
            {
                _assemblySnapField.value = "15";
            }
            if (_environmentTempSlider != null) _environmentTempSlider.RegisterValueChangedCallback(evt => { _environmentSpec.TemperatureC = evt.newValue; });
            if (_environmentVibrationSlider != null) _environmentVibrationSlider.RegisterValueChangedCallback(evt => { _environmentSpec.VibrationAmplitude = evt.newValue; });
            if (_environmentVibrationFreqSlider != null) _environmentVibrationFreqSlider.RegisterValueChangedCallback(evt => { _environmentSpec.VibrationFrequencyHz = evt.newValue; });
            if (_environmentFrictionSlider != null) _environmentFrictionSlider.RegisterValueChangedCallback(evt => { _environmentSpec.SurfaceFriction = evt.newValue; });
            if (_wireApplyBtn != null) _wireApplyBtn.clicked += ApplyWireDefaultsToSelection;
            ...
            if (_leftTabAssemblyBtn != null) _leftTabAssemblyBtn.clicked += () => SetLeftPanelTab(LeftPanelTab.Assembly);
            if (_leftTabEnvironmentBtn != null) _leftTabEnvironmentBtn.clicked += () => SetLeftPanelTab(LeftPanelTab.Environment);
            if (_leftTabWiringBtn != null) _leftTabWiringBtn.clicked += () => SetLeftPanelTab(LeftPanelTab.Wiring);
        ...
        private void SetLeftPanelTab(LeftPanelTab tab)
        {
            ...
            SetPanelVisible(_leftPanelAssembly, tab == LeftPanelTab.Assembly);
            SetPanelVisible(_leftPanelEnvironment, tab == LeftPanelTab.Environment);
            SetPanelVisible(_leftPanelWiring, tab == LeftPanelTab.Wiring);
            ...
            ToggleActiveClass(_leftTabAssemblyBtn, tab == LeftPanelTab.Assembly);
            ToggleActiveClass(_leftTabEnvironmentBtn, tab == LeftPanelTab.Environment);
            ToggleActiveClass(_leftTabWiringBtn, tab == LeftPanelTab.Wiring);
        }
        ...
        private void LoadSession()
        {
            ...
            // RobotStudio package defaults
            _assemblySpec = _robotPackage?.Assembly ?? new AssemblySpec();
            _environmentSpec = _robotPackage?.Environment ?? new EnvironmentSpec();

            EnsureAssemblyRoot();
            ApplyEnvironmentToUi();
            PopulateAssemblyFromSpec();
            EnsureSimulationHost();
            var package = BuildRobotPackageFromState();
            _robotPackage = package;
            _simulationHost?.LoadPackage(package);
            ResetHistory();
        }
        ...
        private void RefreshHierarchy()
        {
            ...
            foreach (var item in _assemblyItems.Values.OrderBy(i => i.ComponentType))
            {
        ...
        private void OnViewportPointerDown(PointerDownEvent evt)
        {
            ...
            if (!_is2dView && evt.button == 0 && HandleAssemblyPointerDown(pointerPos, evt))
            {
                evt.StopPropagation();
                return;
            }
        ...
        private void OnViewportPointerMove(PointerMoveEvent evt)
        {
            ...
            if (!_is2dView && HandleAssemblyPointerMove(pointerPos, delta))
            {
                evt.StopPropagation();
                return;
            }
        ...
        {
            if (_viewportPointerId != -1 && evt.pointerId != _viewportPointerId) return;
            if (!_is2dView && HandleAssemblyPointerUp(evt))
            {
                evt.StopPropagation();
                return;
            }
        ...
        private bool HandleAssemblyPointerDown(Vector2 pointerPos, PointerDownEvent evt)
        {
            if (_studioView == null || _assemblyRoot == null) return false;
            if (!TryGetViewportPoint(pointerPos, out var viewportPoint)) return false;

            if (_pendingCatalogDrag)
            {
                PlacePendingAssemblyItem(viewportPoint);
                return true;
            }

            if (!TryPickAssemblyItem(viewportPoint, out var hitItem)) return false;

            SelectAssemblyItem(hitItem, evt.shiftKey);
            if (_assemblyTool == AssemblyTool.Nail)
            {
                ToggleAssemblyPinned(hitItem);
                return true;
            }

            if (_assemblyTool == AssemblyTool.Move)
            {
                if (TryGetAssemblyPlanePoint(viewportPoint, out var hitPoint))
                {
                    _assemblyDragOffset = hitItem.transform.position - hitPoint;
                    _assemblyDragging = true;
                    _activeAssemblyItem = hitItem;
                }
                return true;
            }

            if (_assemblyTool == AssemblyTool.Rotate)
            {
                _assemblyRotating = true;
                _activeAssemblyItem = hitItem;
                _assemblyRotateAnchor = pointerPos;
                _assemblyRotateStartYaw = hitItem.transform.eulerAngles.y;
                return true;
            }
        ...
        private bool HandleAssemblyPointerMove(Vector2 pointerPos, Vector2 delta)
        {
            if (_studioView == null || _assemblyRoot == null) return false;
            if (_pendingCatalogDrag)
            {
            ...
            if (_assemblyDragging && _activeAssemblyItem != null)
            {
                if (TryGetViewportPoint(pointerPos, out var viewportPoint) &&
                    TryGetAssemblyPlanePoint(viewportPoint, out var hitPoint))
                {
                    var target = hitPoint + _assemblyDragOffset;
                    if (IsAssemblySnapEnabled())
                    {
                        target = SnapToGrid(target);
                    }
                    _activeAssemblyItem.transform.position = target;
                    MarkAssemblyDirty();
                }
                return true;
            }

            if (_assemblyRotating && _activeAssemblyItem != null)
            {
                float yawDelta = delta.x * 0.35f;
                float yaw = _assemblyRotateStartYaw + yawDelta;
                if (IsAssemblySnapEnabled())
                {
                    yaw = SnapAngle(yaw, _rotationSnapDegrees);
                }
                var euler = _activeAssemblyItem.transform.eulerAngles;
                euler.y = yaw;
                _activeAssemblyItem.transform.eulerAngles = euler;
                MarkAssemblyDirty();
                return true;
            }
        ...
        private bool HandleAssemblyPointerUp(PointerUpEvent evt)
        {
            if (_pendingCatalogDrag)
            {
                _pendingCatalogDrag = false;
                ClearPlacementGhost();
                return true;
            }

            if (_assemblyDragging || _assemblyRotating)
            {
                _assemblyDragging = false;
                _assemblyRotating = false;
                _activeAssemblyItem = null;
                return true;
            }
        ...
        private void SetAssemblyTool(AssemblyTool tool)
        {
            _assemblyTool = tool;
            ToggleActiveClass(_assemblySelectBtn, tool == AssemblyTool.Select);
            ToggleActiveClass(_assemblyMoveBtn, tool == AssemblyTool.Move);
            ToggleActiveClass(_assemblyRotateBtn, tool == AssemblyTool.Rotate);
            ToggleActiveClass(_assemblyNailBtn, tool == AssemblyTool.Nail);
        }

        private void UpdateAssemblySnap()
        {
            if (_assemblySnapField == null) return;
            if (float.TryParse(_assemblySnapField.value, NumberStyles.Float, CultureInfo.InvariantCulture, out var value))
            {
                _rotationSnapDegrees = Mathf.Clamp(value, 1f, 90f);
            }
        }

        private bool IsAssemblySnapEnabled()
        {
            return _assemblySnapToggle != null && _assemblySnapToggle.value;
        }

        private void EnsureAssemblyRoot()
        {
            if (_assemblyRoot != null) return;
            _assemblyRoot = new GameObject("RobotAssemblyRoot");
            if (_studioView != null)
            {
                _studioView.SetModelInstance(_assemblyRoot, false);
            }

            var floor = GameObject.CreatePrimitive(PrimitiveType.Plane);
            floor.name = "AssemblyFloor";
            floor.transform.SetParent(_assemblyRoot.transform, false);
            floor.transform.localScale = new Vector3(0.2f, 1f, 0.2f);
            var renderer = floor.GetComponent<Renderer>();
            if (renderer != null)
            {
                if (_assemblyFloorMaterial == null)
                {
                    _assemblyFloorMaterial = CreateUnlitMaterial(new Color(0.08f, 0.1f, 0.14f, 1f), "RobotStudio_AssemblyFloor");
                }
                renderer.sharedMaterial = _assemblyFloorMaterial;
            }
        }

        private void EnsureSimulationHost()
        {
            if (_simulationHost != null) return;
            _simulationHost = FindFirstObjectByType<RobotStudioSimulationHost>();
            if (_simulationHost != null)
            {
                if (_simulationHost.GetComponent<RobotStudioDiagnosticsOverlay>() == null)
                {
                    _simulationHost.gameObject.AddComponent<RobotStudioDiagnosticsOverlay>();
                }
                return;
            }
            var hostObject = new GameObject("RobotStudioSimulationHost");
            _simulationHost = hostObject.AddComponent<RobotStudioSimulationHost>();
            hostObject.AddComponent<RobotStudioDiagnosticsOverlay>();
        }

        private void ClearAssemblyItems()
        {
            foreach (var item in _assemblyItems.Values)
            {
                if (item != null)
                {
        ...
        private void PopulateAssemblyFromSpec()
        {
            ...
            {
                var item = CreateAssemblyItem(part.ComponentType, part.CatalogId, ToVector3(part.Position), Quaternion.Euler(ToVector3(part.Rotation)));
                if (item == null) continue;
                item.InstanceId = part.InstanceId;
                item.ComponentType = part.ComponentType;
                item.CatalogId = part.CatalogId;
                item.DensityKgPerM3 = (float)part.DensityKgPerM3;
                item.Friction = (float)part.Friction;
                item.MassKg = (float)part.MassKg;
                item.transform.localScale = ToVector3(part.Scale);
                item.SetPinned(part.Pinned);
            }
        }

        private void ApplyEnvironmentToUi()
        {
            if (_environmentSpec == null) return;
            if (_environmentTempSlider != null) _environmentTempSlider.SetValueWithoutNotify((float)_environmentSpec.TemperatureC);
            if (_environmentVibrationSlider != null) _environmentVibrationSlider.SetValueWithoutNotify((float)_environmentSpec.VibrationAmplitude);
            if (_environmentVibrationFreqSlider != null) _environmentVibrationFreqSlider.SetValueWithoutNotify((float)_environmentSpec.VibrationFrequencyHz);
            if (_environmentFrictionSlider != null) _environmentFrictionSlider.SetValueWithoutNotify((float)_environmentSpec.SurfaceFriction);
        }

        private void BeginCatalogPlacement(ComponentCatalog.Item item)
        {
            _pendingCatalogItem = item;
            _pendingCatalogDrag = true;
            CreatePlacementGhost();
        }

        private void PlacePendingAssemblyItem(Vector2 viewportPoint)
        {
            if (!_pendingCatalogDrag) return;
            if (!TryGetAssemblyPlanePoint(viewportPoint, out var hitPoint)) return;

            var newItem = CreateAssemblyItem(_pendingCatalogItem.Type, _pendingCatalogItem.Id, hitPoint, Quaternion.identity);
            if (newItem != null)
            {
                SelectAssemblyItem(newItem, false);
            }
        ...
        private void UpdatePlacementGhost(Vector2 pointerPos)
        {
            if (_placementGhost == null || !_pendingCatalogDrag) return;
            if (!TryGetViewportPoint(pointerPos, out var viewportPoint)) return;
            if (!TryGetAssemblyPlanePoint(viewportPoint, out var hitPoint)) return;
            _placementGhost.transform.position = hitPoint;
        }
        ...
        private void CreatePlacementGhost()
        {
            ...
            _placementGhost.transform.localScale = Vector3.one * 0.02f;
            var collider = _placementGhost.GetComponent<Collider>();
            if (collider != null) Destroy(collider);
            var renderer = _placementGhost.GetComponent<Renderer>();
            if (renderer != null)
            {
                if (_placementGhostMaterial == null)
                {
                    _placementGhostMaterial = CreateUnlitMaterial(new Color(0.35f, 0.75f, 1f, 0.65f), "RobotStudio_PlacementGhost");
                }
                renderer.sharedMaterial = _placementGhostMaterial;
            }
        }
        ...
        private RobotStudioAssemblyItem CreateAssemblyItem(string type, string catalogId, Vector3 position, Quaternion rotation)
        {
            EnsureAssemblyRoot();
            var root = new GameObject($"Assembly_{type}");
            root.transform.SetParent(_assemblyRoot.transform, false);
            root.transform.position = position;
            root.transform.rotation = rotation;

            var item = root.AddComponent<RobotStudioAssemblyItem>();
            item.InstanceId = Guid.NewGuid().ToString("N");
            item.ComponentType = type ?? string.Empty;
            item.CatalogId = catalogId ?? string.Empty;

            _assemblyItems[item.InstanceId] = item;

            var indicator = GameObject.CreatePrimitive(PrimitiveType.Cylinder);
            indicator.name = "PinIndicator";
            indicator.transform.SetParent(root.transform, false);
            indicator.transform.localScale = new Vector3(0.01f, 0.005f, 0.01f);
            indicator.transform.localPosition = Vector3.zero;
            var indicatorRenderer = indicator.GetComponent<Renderer>();
            if (indicatorRenderer != null)
            {
                if (_pinIndicatorMaterial == null)
                {
                    _pinIndicatorMaterial = CreateUnlitMaterial(new Color(1f, 0.3f, 0.3f, 0.9f), "RobotStudio_PinIndicator");
                }
                indicatorRenderer.sharedMaterial = _pinIndicatorMaterial;
            }
            var indicatorCollider = indicator.GetComponent<Collider>();
            if (indicatorCollider != null) Destroy(indicatorCollider);
            item.PinIndicator = indicator;
            item.SetPinned(false);

            LoadAssemblyModel(item, type);
            return item;
        }

        private async void LoadAssemblyModel(RobotStudioAssemblyItem item, string type)
        {
            if (item == null) return;
            string modelPath = ResolveCatalogModelPath(type);
            if (string.IsNullOrWhiteSpace(modelPath))
            {
                CreateAssemblyPlaceholder(item);
                return;
            }

            using var tokenSource = new CancellationTokenSource();
            var model = await RuntimeModelLoader.TryLoadModelAsync(modelPath, item.gameObject, tokenSource.Token);
            if (model == null)
            {
                CreateAssemblyPlaceholder(item);
                return;
            }
            item.MassKg = ComputeAssemblyMass(item.transform, item.DensityKgPerM3);
        }

        private void CreateAssemblyPlaceholder(RobotStudioAssemblyItem item)
        {
            if (item == null) return;
            var cube = GameObject.CreatePrimitive(PrimitiveType.Cube);
            cube.name = "AssemblyPlaceholder";
            cube.transform.SetParent(item.transform, false);
            cube.transform.localScale = new Vector3(0.02f, 0.01f, 0.02f);
            item.MassKg = ComputeAssemblyMass(item.transform, item.DensityKgPerM3);
        }

        private string ResolveCatalogModelPath(string type)
        {
            if (string.IsNullOrWhiteSpace(type)) return string.Empty;
            var item = ComponentCatalog.GetByType(type);
            if (string.IsNullOrWhiteSpace(item.Type)) return string.Empty;
            if (string.IsNullOrWhiteSpace(item.ModelFile)) return string.Empty;

            if (!string.IsNullOrWhiteSpace(item.SourcePath) &&
                ComponentPackageUtility.IsPackagePath(item.SourcePath) &&
                ComponentPackageUtility.TryExtractEntryToCache(item.SourcePath, item.ModelFile, out var extracted))
            {
                return extracted;
            }

            return item.ModelFile;
        }

        private float ComputeAssemblyMass(Transform root, float density)
        {
            ...
            return Mathf.Max(0f, volume * density);
        }

        private bool TryPickAssemblyItem(Vector2 viewportPoint, out RobotStudioAssemblyItem item)
        {
            item = null;
            if (_studioView == null) return false;
            if (!_studioView.TryGetViewportRay(viewportPoint, out var ray)) return false;
            if (!Physics.Raycast(ray, out var hit, 100f)) return false;
            item = hit.collider != null ? hit.collider.GetComponentInParent<RobotStudioAssemblyItem>() : null;
            return item != null;
        }

        private bool TryGetAssemblyPlanePoint(Vector2 viewportPoint, out Vector3 point)
        {
            point = Vector3.zero;
            if (_studioView == null) return false;
            if (!_studioView.TryGetViewportRay(viewportPoint, out var ray)) return false;
        ...
        private void SelectAssemblyItem(RobotStudioAssemblyItem item, bool additive)
        {
            if (item == null) return;
            if (!additive)
            {
            ...
            if (!_selectedAssemblyItems.Contains(item))
            {
                _selectedAssemblyItems.Add(item);
            }
            _activeAssemblyItem = item;
            _studioView?.SetSelectionOutline(item.transform);
            _studioView?.SetPartFocus(item.transform);
        }

        private void ToggleAssemblyPinned(RobotStudioAssemblyItem item)
        {
            if (item == null) return;
        ...
        {
            float x = Mathf.Round(point.x / _breadboardPitch) * _breadboardPitch;
            float z = Mathf.Round(point.z / _breadboardPitch) * _breadboardPitch;
            return new Vector3(x, point.y, z);
        }

        private static float SnapAngle(float value, float snap)
        {
        ...
        private void FrameAssembly()
        {
            _studioView?.FrameModel();
        }

        private void AutoArrangeBreadboard()
        {
            EnsureAssemblyRoot();
        ...
        private Bounds GetItemBounds(Transform root)
        {
            ...
            {
                return new Bounds(root.position, Vector3.one * 0.02f);
            }
            Bounds bounds = renderers[0].bounds;
            for (int i = 1; i < renderers.Length; i++)
            {
        ...
        private void ApplyWireDefaultsToSelection()
        {
            var spec = _assemblySpec ?? new AssemblySpec();
            if (!_selectedAssemblyItems.Any())
            {
                _assemblySpec = spec;
                return;
            }

            double young = ReadDouble(_wireYoungField, 2.0e9);
            double maxStress = ReadDouble(_wireMaxStressField, 2.0e8);
            double damping = ReadDouble(_wireDampingField, 0.2);
            foreach (var wire in spec.Wires)
            {
                wire.YoungModulus = young;
                wire.MaxStress = maxStress;
                wire.Damping = damping;
            }
            _assemblySpec = spec;
            MarkAssemblyDirty();
        }

        private RobotStudioPackage BuildRobotPackageFromState()
        {
            var package = _robotPackage ?? new RobotStudioPackage();
            package.Assembly = BuildAssemblySpecFromScene();
            package.Environment = _environmentSpec ?? new EnvironmentSpec();
            package.Metadata ??= new Dictionary<string, object>();
            package.Robot ??= new RobotSpec { Name = "Robot" };
            package.Circuit ??= new CircuitSpec();
            return package;
        }

        private AssemblySpec BuildAssemblySpecFromScene()
        {
            var spec = new AssemblySpec();
            var parts = new List<AssemblyPartSpec>();
            foreach (var item in _assemblyItems.Values)
            {
                if (item == null) continue;
                parts.Add(new AssemblyPartSpec
                {
                    InstanceId = item.InstanceId,
                    ComponentType = item.ComponentType,
                    CatalogId = item.CatalogId,
                    Position = ToVec3(item.transform.position),
                    Rotation = ToVec3(item.transform.eulerAngles),
                    Scale = ToVec3(item.transform.localScale),
                    Pinned = item.Pinned,
        ...
        private void SaveRobotPackage()
        {
            if (string.IsNullOrWhiteSpace(_robotPackagePath))
            {
                SaveRobotPackageAs();
                return;
            }
            SaveRobotPackageToPath(_robotPackagePath);
        }

        private void SaveRobotPackageAs()
        {
#if UNITY_EDITOR
            string baseName = "robot";
            string initialDir = ComponentCatalog.GetUserComponentRoot();
            string chosen = EditorUtility.SaveFilePanel("Save Robot Package", initialDir, baseName, "rtrobot");
            if (string.IsNullOrWhiteSpace(chosen)) return;
            if (!chosen.EndsWith(RobotStudioSerializer.PackageExtension, StringComparison.OrdinalIgnoreCase))
            {
                chosen += RobotStudioSerializer.PackageExtension;
            }
            SaveRobotPackageToPath(chosen);
#else
            SetStatus("Save As not available at runtime. Paste the path manually.", true);
#endif
        }

        private void SaveRobotPackageToPath(string filePath)
        {
            var package = BuildRobotPackageFromState();
            SimulationSerializer.SaveRobotPackage(package, filePath);
            _robotPackagePath = filePath;
            _robotPackage = package;
            SetStatus("Saved robot package.", false);
            MarkSavedSnapshot(BuildSnapshot());
        }

        private void OpenRobotPackageFromDialog()
        {
#if UNITY_EDITOR
            string root = ComponentCatalog.GetUserComponentRoot();
            string file = EditorUtility.OpenFilePanel("Open Robot Package", root, "rtrobot");
            if (string.IsNullOrWhiteSpace(file)) return;
            LoadRobotPackageFromPath(file);
#else
            SetStatus("Open not available at runtime. Paste the path manually.", true);
#endif
        }

        private void LoadRobotPackageFromPath(string filePath)
        {
            var package = SimulationSerializer.LoadRobotPackage(filePath, true) ?? new RobotStudioPackage();
            _robotPackagePath = filePath;
            _robotPackage = package;
            _assemblySpec = package.Assembly ?? new AssemblySpec();
            _environmentSpec = package.Environment ?? new EnvironmentSpec();
            EnsureAssemblyRoot();
            ClearAssemblyItems();
            PopulateAssemblyFromSpec();
            ApplyEnvironmentToUi();
            EnsureSimulationHost();
            _simulationHost?.LoadPackage(_robotPackage);
            SetStatus($"Loaded robot package: {Path.GetFileName(filePath)}", false);
            MarkSavedSnapshot(BuildSnapshot());
        }

        private static double ReadDouble(TextField field, double fallback)
        {
            if (field == null) return fallback;
            if (double.TryParse(field.value, NumberStyles.Float, CultureInfo.InvariantCulture, out var value))
            {
                return value;
            }
            return fallback;
