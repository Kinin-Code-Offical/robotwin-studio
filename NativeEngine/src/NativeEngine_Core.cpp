...
namespace
{
  ...
  std::unordered_map<std::uint64_t, std::shared_ptr<AnalogDriver>> g_analogDrivers;
  std::uint32_t g_hiddenNextId = 1000000u;

  std::uint64_t MakeAnalogDriverKey(int avrIndex, int pinIndex)
  {
    return (static_cast<std::uint64_t>(static_cast<std::uint32_t>(avrIndex)) << 32) |
...
extern "C"
{
  ...
  UNITY_EXPORT int SetAnalogVoltageForAvr(int avrIndex, int pinIndex, float voltage)
  {
    ...
    const std::uint64_t key = MakeAnalogDriverKey(avrIndex, pinIndex);
    auto it = g_analogDrivers.find(key);
    if (it == g_analogDrivers.end() || !it->second)
    {
      auto driver = std::make_shared<AnalogDriver>(g_hiddenNextId++, static_cast<double>(voltage));
      driver->Connect(0, nodeId);
      ctx.AddComponent(driver);
      g_analogDrivers[key] = driver;
      return 1;
    }

    it->second->SetVoltage(static_cast<double>(voltage));
    if (it->second->m_node != nodeId)
    {
      it->second->Connect(0, nodeId);
    }
    return 1;
  }
