...
public:
    ...
    void flush()
    {
        ...
        case ErrorLogging::EVENT_LINE_LOST:
            Serial.print("LINE_LOST pos=");
            Serial.print(e.value1); // Removed 2nd arg for float precision to avoid ambiguity for now or add overload
            Serial.print(" conf=");
            Serial.println(e.value2);
            break;
        case ErrorLogging::EVENT_OBSTACLE_HIT:
            Serial.print("OBSTACLE pwm=");
            Serial.print(e.value1);
            Serial.print(" stallTime=");
            Serial.println(e.value2);
            break;
        case ErrorLogging::EVENT_RECOVERY_START:
            Serial.print("RECOVERY_START strategy=");
            Serial.println(e.decisionCode);
            break;
        case ErrorLogging::EVENT_PROBABILISTIC_CHOICE:
            Serial.print("DECISION: strategy=");
    ...
    bool hasEvents() { return (rd != wr) || full; }
};
...
public:
    ...
    {
        return !filteredValues_[0] && !filteredValues_[1] && !filteredValues_[2] && !filteredValues_[3];
    }
...
public:
    ...
    float fuseGhostAndReal(float realPosition)
    {
        ...
        float decayFactor = exp(-age / MapNavigation::CONFIDENCE_HALF_LIFE_MS);
...
private:
    ...
    DTConfig::Action lastAction = DTConfig::ACTION_IDLE;
...
public:
     ...
     */
    DTConfig::Action evaluate(float position, float confidence, bool oscillating)
    {
        // NODE 1: Line center?
        if (abs(position) < DTConfig::NODE_CENTER_TOLERANCE)
        {
            // NODE 2: Confidence check
            if (confidence > DTConfig::NODE_HIGH_CONFIDENCE)
            {
                verifyCounter = 0;
                return DTConfig::ACTION_IDLE; // Hiçbir şey yapma, mükemmel!
            }
            else if (confidence > DTConfig::NODE_MED_CONFIDENCE)
            {
                // Verify: Birkaç cycle daha kontrol et
                if (verifyCounter < DTConfig::VERIFY_CYCLES)
                {
                    verifyCounter++;
                    return DTConfig::ACTION_VERIFY;
                }
                else
                {
                    verifyCounter = 0;
                    return DTConfig::ACTION_IDLE; // Doğrulandı
                }
            }
            else
            {
                // Low confidence, ama center → gentle correction
                return DTConfig::ACTION_GENTLE_TURN;
            }
        }
        else
        {
            ...
            if (oscillating)
            {
                ...
                return DTConfig::ACTION_IDLE; // Salınıma izin verme
            }

            // NODE 3: How far?
            float distance = abs(position);

            if (distance < DTConfig::NODE_NEAR_THRESHOLD)
            {
                return DTConfig::ACTION_GENTLE_TURN;
            }
            else if (distance < DTConfig::NODE_FAR_THRESHOLD)
            {
                return DTConfig::ACTION_SHARP_TURN;
            }
            else
            {
                // Çok uzak → emergency
                return DTConfig::ACTION_EMERGENCY;
            }
        }
    }
     ...
     */
    float getOutputScale(DTConfig::Action action)
    {
        switch (action)
        {
        case DTConfig::ACTION_IDLE:
            return 0.0f; // Hiçbir düzeltme yapma
        case DTConfig::ACTION_VERIFY:
            return 0.2f; // Çok hafif düzeltme
        case DTConfig::ACTION_GENTLE_TURN:
            return 0.6f; // Normal düzeltme
        case DTConfig::ACTION_SHARP_TURN:
            return 1.0f; // Tam güç
        case DTConfig::ACTION_EMERGENCY:
