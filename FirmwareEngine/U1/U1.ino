...
void loop()
{
    ...
    uint8_t act = tree.evaluate(posFuse, 85, oscFlg);                        // conf=85% hardcode
    uint8_t spdMul = (uint8_t)((map.getRecommendedSpeed() * 100.0f) + 0.5f); // 0-100%, rounded
    ...
    if (st == S_LINE)
    {
        ...
        {
            pidOut = (int16_t)(pid.compute(0, posW, 0.004f) * tree.getOutputScale((DTConfig::Action)act));
            lSpd = (MotorPhysics::LEFT_BASE_PWM + MotorPhysics::RIGHT_BASE_PWM) >> 1;
            lSpd = (lSpd * spdMul) >> 6; // Fast divide by 64 (approximates /100)
            aSpd = pidOut;
        }
    }
    ...
    case S_LINE:
    {
        ...
        if ((cnt & 0x04) == 0)
        { // Fast: cnt % 5 approximated as cnt % 4
            // Odometry: PWM → mm/cycle (pre-computed scaling)
            uint16_t dMM = (uint16_t)(lSpd >> 7); // Fast: lSpd / 128 ≈ mm/cycle
            static float hdg = 0;
            ...
            if (map.isTurnAhead())
            {
                log.log((ErrorLogging::EventType)9, hdg, spdMul / 100.0f); // EVENT_TURN_PREDICTED
            }
        }
        ...
        if (oscFlg)
        {
            log.log((ErrorLogging::EventType)10, posFuse, 85, act); // EVENT_CONFIDENCE_LOW
        }

        // Line loss
        if (sns.isOffLine())
        {
            log.log((ErrorLogging::EventType)0, pos, pidOut); // EVENT_LINE_LOST

            if (abs(pos) < 30 && abs(pidOut) < 50)
            {
                log.log((ErrorLogging::EventType)6, 87, 15, 0); // CONTINUE
            }
            else
            {
                drv.spin(pidOut > 0 ? 50 : -50);
                log.log((ErrorLogging::EventType)6, 73, 30, 1); // QUICK_TURN
            }
        }
    ...
    case S_TURN:
    {
        ...
        { // timeout
            ...
            tStateStart = millis();
            errCnt++;
            log.log((ErrorLogging::EventType)0, pos, 999); // TURN_TIMEOUT
