// U1.cpp - Main Control | 250Hz sync pipeline
// Ported from U1.ino

#include "Config.h" // Config includes Arduino.h
#include <AFMotor.h>
#include <Servo.h>
...
void loop()
{
    ...
    // Simulator fix: Don't check strictly for min time, just return if too fast?
    // In HLE, the loop is called by main.cpp which might drive the clock.
    // If g_millis updates slowly, this might hang.
    ...
    // while((micros()-t0) < 1000); // sync wait -> REMOVE BUSY WAIT FOR SIM
    ...
    DTConfig::Action act = tree.evaluate(posFuse, 85, oscFlg);   // conf=85% hardcode
    ...
    case S_LINE:
    {
        ...
        if (cnt % 5 == 0)
        {
            ...
            map.updateCurrentCell((MapNavigation::CellType)cType, posFuse);

            if (map.isTurnAhead())
            {
                sysLog.log(ErrorLogging::EVENT_TURN_PREDICTED, hdg, spdMul / 100.0f);
            }
        }
        ...
        if (oscFlg)
        {
            sysLog.log(ErrorLogging::EVENT_CONFIDENCE_LOW, posFuse, 85, act);
        }

        // Line loss
        if (sns.isOffLine())
        {
            sysLog.log(ErrorLogging::EVENT_LINE_LOST, pos, pidOut);

            if (abs(pos) < 30 && abs(pidOut) < 50)
            {
                sysLog.log(ErrorLogging::CONTINUE, 87, 15, 0);
            }
            else
            {
                drv.spin(pidOut > 0 ? 50 : -50);
                sysLog.log(ErrorLogging::QUICK_TURN, 73, 30, 1);
            }
        }
    ...
    case S_TURN:
    {
        ...
        { // timeout
            ...
            tStateStart = millis();
            errCnt++;
            sysLog.log(ErrorLogging::TURN_TIMEOUT, pos, 999);
    ...
    if (sysLog.hasEvents() && !(cnt % 10))
    {
        for (uint8_t i = 0; i < 3 && sysLog.hasEvents(); i++)
            sysLog.flush();
    }
